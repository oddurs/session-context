 ...
 */
export function MethodEntry({ method }: { method: Method }) {
  const rows = [
  ...
  return (
    <article className="border-t border-rule py-5 first:border-t-0 first:pt-3">
      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
        <h3 className="text-lg font-medium leading-snug tracking-tight">{method.name}</h3>
        <span className="shrink-0 text-sm text-ink-faint">{method.standing}</span>
      </div>

      <dl className="mt-3 grid gap-x-6 gap-y-2 sm:grid-cols-[6.5rem_minmax(0,1fr)]">
        {rows.map((row) => (
      ...
      {method.section && (
        <div className="mt-3 sm:pl-[8.5rem]">
          <a
            href={`/#${method.section}`}
